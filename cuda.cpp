...
#pragma omp parallel for num_threads(8)
    ...
    // (matrixSize + THREADS_DIM - 1 ) / THREADS_DIM
    ...
    for (int e = 0; e < EPOCHS; e++) {
        ...
        dim3 matTrDimGrid((1 + matTrDimBlock.x - 1) / matTrDimBlock.x,
                          (numTrainData + matTrDimBlock.y - 1) / matTrDimBlock.y);
        /*dim3 blockSize (TILE_DIM, TILE_DIM);
        dim3 gridSize (gridDimMult, gridDimMult);*/
        kernel_matMultiply<<< matTrDimGrid, matTrDimBlock >>>(
        ...
        // 2.a
        float *deltaBack, *d_deltaBack, *d_deltaTrainArray;//  = new float[numTrainData];
        cudaMallocHost(&deltaBack, numTrainData * sizeof(float));
        cudaMalloc(&d_deltaBack, numTrainData * sizeof(float));
        cudaMalloc(&d_deltaTrainArray, numTrainPoints * sizeof(float));
        ...
        kernel_matMultiply<<< matTrDimGrid, matTrDimBlock >>>(
                d_deltaBack, d_nnWeightArray, d_deltaTrainArray, numTrainData, 1, 1, 180);
        ...
        dim3 trDimGrid(180 / TILE_DIM, numTrainData / TILE_DIM, 1);
        dim3 trDimBlock(TILE_DIM, TILE_DIM, 1);
        kernel_matTranspose<<< trDimGrid, trDimBlock >>>(d_mt, d_trainArray, numTrainData, 180);
        ...
        dim3 blk(TILE_DIM, TILE_DIM, 1);
        dim3 grd((1 + blk.x - 1) / blk.x, (180 + blk.y - 1) / blk.y);
        kernel_matMultiply<<< grd, blk >>>(
                d_mt, d_deltaBack, d_deltaWeightArray, 180, numTrainData, numTrainData, 1);
    ...
    cudaMallocHost(&predictedArray, numTestData * sizeof(float));
    cudaMalloc(&d_predictedArray, numTestData * sizeof(float));
    // matMultiply(testArray, nnWeightArray, predictedArray, numTestData, 180, 180, 1);
    dim3 g( (1+TILE_DIM-1)/TILE_DIM, (numTestData+TILE_DIM-1)/TILE_DIM);
    dim3 b(TILE_DIM, TILE_DIM);
    kernel_matMultiply<<< g, b >>>(d_testArray, d_nnWeightArray, d_predictedArray, numTestData, 180, 180, 1);
    cudaMemcpy(predictedArray, d_predictedArray, numTestData, cudaMemcpyDeviceToHost);
    int numRightGuess = 0;
